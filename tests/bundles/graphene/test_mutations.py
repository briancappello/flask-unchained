...
class TestParentMutations:
    ...
    def test_create_parent_works_without_children(self, graphql_client):
        ...
        assert dict(result) == dict(
            id=str(parent.id), name=parent.name, children=parent.children
        )
    ...
    def test_create_parent_works_with_children(self, graphql_client):
        ...
        assert dict(result) == dict(
            id=str(parent.id),
            name=parent.name,
            children=[{"id": str(child.id)} for child in children],
        )
    ...
    def test_edit_parent_name(self, graphql_client):
        ...
        assert dict(result) == dict(
            id=str(parent.id),
            name="new-name",
            children=[{"id": str(child.id)} for child in parent.children],
        )

    def test_edit_parent_children(self, graphql_client):
        parent = parent_manager.get_by(name="parent_one")
        children = child_manager.filter(
            ~child_manager.Meta.model.id.in_(child.id for child in parent.children)
        )
        result = graphql_client.execute(
            EDIT_PARENT, dict(id=parent.id, children=[child.id for child in children])
        )
