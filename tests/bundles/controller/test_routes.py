...
class TestController:
    ...
    def test_it_does_not_mutate_existing_routes(self):
        ...
        orig_routes = [
            route
            for routes in getattr(SiteController, CONTROLLER_ROUTES_ATTR).values()
            for route in routes
        ]
...
class TestResource:
    ...
    def test_it_renames_with_deeply_customized_member_params(self):
        routes = list(
            resource(
                ...
                subresources=[resource(RoleResource, member_param="<string:slug>")],
            )
        )
    ...
    def test_member_param_overrides_unique_member_params(self):
        routes = list(
            resource(
                "/baz",
                BazResource,
                unique_member_param="<int:pk>",
                subresources=[resource(BarResource, member_param="<int:pk>")],
            )
        )
    ...
    def test_differently_named_deeply_customized_member_params(self):
        routes = list(
            resource(
                UserResource,
                member_param="<string:slug>",
                subresources=[resource(RoleResource, member_param="<string:slug2>")],
            )
        )
    ...
    def test_it_does_not_mutate_existing_routes(self):
        ...
        orig_routes = [
            route
            for routes in getattr(UserResource, CONTROLLER_ROUTES_ATTR).values()
            for route in routes
        ]
        assert orig_routes[0].endpoint == routes[0].endpoint
        assert orig_routes[0].rule == "/"
    ...
    def test_it_does_not_mutate_subresource_routes(self):
        ...
        orig_routes = [
            route
            for routes in getattr(RoleResource, CONTROLLER_ROUTES_ATTR).values()
            for route in routes
        ]
