...
class TestApiLogin:
    ...
    def test_json_login_errors(self, api_client):
        r = api_client.post("security_api.login", data=dict(email=None, password=None))
        assert r.json["error"] == "Invalid email and/or password."

    def test_json_login_with_email(self, api_client, user):
        r = api_client.post(
            "security_api.login", data=dict(email=user.email, password="password")
        )
        assert r.status_code == 200
        assert "user" in r.json
    ...
    def test_active_user_required(
        self, api_client, user, session_manager: SessionManager
    ):
    ...
    def test_confirmed_user_required(
        self,
        api_client,
        user,
