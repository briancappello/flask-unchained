...
class TestHtmlSendConfirmationEmail:
    def test_email_required(self, client, templates):
        ...
        assert templates[0].template.name == "security/send_confirmation_email.html"
        assert "Email is required." in r.html, r.html

    def test_cannot_reconfirm(self, user, client, templates):
        r = client.post(
            "security_controller.send_confirmation_email", data=dict(email=user.email)
        )
        assert r.status_code == 200
        assert templates[0].template.name == "security/send_confirmation_email.html"
    ...
    def test_instructions_resent(
        ...
        assert r.status_code == 200
        assert templates[2].template.name == "security/send_confirmation_email.html"
...
class TestApiSendConfirmationEmail:
    ...
    def test_cannot_reconfirm(self, user, api_client):
        r = api_client.post(
            "security_api.send_confirmation_email", data=dict(email=user.email)
        )
        assert r.status_code == 400
    ...
    @pytest.mark.user(confirmed_at=None)
    def test_instructions_resent(
        self, api_client, user, outbox, templates, security_service
    ):
        security_service.register_user(user)
        assert len(outbox) == len(templates) == 1
