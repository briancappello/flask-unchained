...
class TestHtmlChangePassword:
    ...
    def test_min_length(self, client, templates):
        client.login_user()
        r = client.post(
            "security_controller.change_password",
            data=dict(
                password="password", new_password="fail", new_password_confirm="fail"
            ),
        )
        assert r.status_code == 200
        assert templates[0].template.name == "security/change_password.html"
    ...
    def test_new_same_as_the_old(self, client, templates):
        ...
        assert templates[0].template.name == "security/change_password.html"
        assert (
            "Your new password must be different than your previous password." in r.html
        )
