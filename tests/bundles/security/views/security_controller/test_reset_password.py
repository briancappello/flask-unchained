...
class TestHtmlResetPassword:
    def test_anonymous_user_required(
        self, user, client, security_service, password_resets
    ):
    ...
    def test_token_expired(
        self, user, client, security_service, password_resets, outbox, templates
    ):
        security_service.send_reset_password_instructions(user)
        ...
        assert templates[0].context.get("reset_link") != templates[1].context.get(
            "reset_link"
        )
        ...
        error_msg = (
            "You did not reset your password within -1 seconds. "
    ...
    def test_submit_errors(
        self, user, client, security_service, templates, password_resets
    ):
        security_service.send_reset_password_instructions(user)
        token = password_resets[0]["token"]
        ...
        r = client.post(
            ...
            token=token,
            data=dict(password="short", password_confirm="short"),
        )
        assert r.status_code == 200
        ...
        r = client.post(
            "security_controller.reset_password",
            token=token,
            data=dict(password="long enough", password_confirm="but not the same"),
        )
        assert r.status_code == 200
    ...
    def test_valid_submit(
        ...
        r = client.post(
            "security_controller.reset_password",
            token=token,
            data=dict(password="new password", password_confirm="new password"),
        )
        assert r.status_code == 302
        assert r.path == "/"
...
class TestApiResetPassword:
    def test_anonymous_user_required(
        self, user, api_client, security_service, password_resets
    ):
        security_service.send_reset_password_instructions(user)
        token = password_resets[0]["token"]
    ...
    def test_http_get_redirects_to_frontend_form(
        self, user, api_client, security_service, password_resets
    ):
        security_service.send_reset_password_instructions(user)
        assert len(password_resets) == 1
    ...
    @pytest.mark.options(SECURITY_RESET_PASSWORD_WITHIN="-1 seconds")
    def test_token_expired(
        self, user, api_client, security_service, password_resets, outbox, templates
    ):
        security_service.send_reset_password_instructions(user)
        assert len(password_resets) == 1
        token = password_resets[0]["token"]
    ...
    def test_submit_errors(self, user, api_client, security_service, password_resets):
        security_service.send_reset_password_instructions(user)
        token = password_resets[0]["token"]
        ...
        r = api_client.post(
            "security_api.post_reset_password",
            token=token,
            data=dict(password="short", password_confirm="short"),
        )
        assert r.status_code == 400
