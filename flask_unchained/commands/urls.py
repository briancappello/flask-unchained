...
@click.command()
@click.argument("url")
@click.option("--method", default="GET", help="Method for url to match (default: GET)")
@with_appcontext
def url(url: str, method: str):
    """Show details for a specific URL."""
    try:
        url_rule, params = current_app.url_map.bind("localhost").match(
            url, method=method, return_rule=True
        )
