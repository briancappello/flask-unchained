...
class SecurityController(Controller):
    ...
    def login(self):
        ...
        if form.validate_on_submit():
            try:
                ...
                if request.is_json:
                    return self.jsonify(
                        {"token": form.user.get_auth_token(), "user": form.user}
                    )
                self.flash(
                    _("flask_unchained.bundles.security:flash.login"),
                    category="success",
                )
        ...
        if form.errors and request.is_json:
            return self.jsonify(
                {"error": form.errors.get("_error")[0]}, code=HTTPStatus.UNAUTHORIZED
            )

        return self.render(
            "login", login_user_form=form, **self.security.run_ctx_processor("login")
        )
    ...
    @route(methods=["GET", "POST"], only_if=lambda app: app.config.SECURITY_CONFIRMABLE)
    ...
    @route("/confirm/<token>", only_if=lambda app: app.config.SECURITY_CONFIRMABLE)
    def confirm_email(self, token):
        """
        View function to confirm a user's token from the confirmation email send to them.
        Supports html and json requests.
        """
        expired, invalid, user = self.security_utils_service.confirm_email_token_status(
            token
        )
    ...
    @route(methods=["GET", "POST"], only_if=lambda app: app.config.SECURITY_RECOVERABLE)
    @anonymous_user_required(msg="You are already logged in", category="success")
    ...
    def reset_password(self, token):
        ...
        if form.validate_on_submit():
            ...
            if request.is_json:
                return self.jsonify({"token": user.get_auth_token(), "user": user})
            return self.redirect(
    ...
    @route(methods=["GET", "POST"], only_if=lambda app: app.config.SECURITY_CHANGEABLE)
    @auth_required
