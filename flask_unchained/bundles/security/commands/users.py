...
@click.option("--email", prompt="Email address", help="The user's email address.")
@click.option(
    "--password",
    prompt="Password",
...
def create_user(email, password, active, confirmed_at, role, send_email=False):
    ...
    user = user_manager.create(
        email=email, password=password, is_active=active, confirmed_at=confirmed_at
    )
...
@click.option(
    "--role", "-r", default="ROLE_ADMIN", show_default=True, help="The admin role name."
)
@click.option(
    "--send-email/--no-email",
    default=False,
...
@click.argument(
    "query",
    nargs=1,
    help="The query to search for a user by. For example, "
...
@click.argument(
    "query",
    help="The query to search for a user by. For example, `id=5`, "
...
def confirm_user(query):
    ...
    if click.confirm(f"Are you sure you want to confirm {user!r}?"):
        if security_service.confirm_user(user):
            click.echo(
                f"Successfully confirmed {user!r} at "
